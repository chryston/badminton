...
async def remove_roster_entry(session_id: UUID, entry_id: UUID, _=Depends(require_admin)):
...
async def verify_payment(session_id: UUID, entry_id: UUID, _=Depends(require_admin)):
    try:
